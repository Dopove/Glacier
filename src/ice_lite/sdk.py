...
class InfiniteContextClient:
    ...
    async def ingest(self, file_or_dir_path: str, x_session_id: str, x_user_id: str = DEFAULT_USER_ID) -> Union[bool, int]:
        """
        ...
        if os.path.isdir(file_or_dir_path):
            return await self.ingestion.ingest_directory(file_or_dir_path, x_session_id, x_user_id, DEFAULT_TENANT_ID)
        elif os.path.isfile(file_or_dir_path):
            return await self.ingestion.ingest_file(file_or_dir_path, x_session_id, x_user_id, DEFAULT_TENANT_ID)
        else:
...
class CompletionsInterface:
    ...
    async def create(
        ...
        stream: bool = False,
        local_inference_func: Optional[Callable[[List[Dict[str, Any]]], dict]] = None,
        **kwargs
        ...
        for i in range(max_continuations + 1):
            ...
            if i == 0: 
                # Detect the role of the last message to save it correctly
                ...
                if role in ["user", "tool", "assistant"]:
                    await self.client.episodic.save_message(
                        ...
                        role=role, content=last_msg.get("content"), metadata=last_msg.get("metadata")
                    )
            ...
            if user_message_content:
                # Mock keyword retrieval for LITE version
                query_vector = await self.client.embedder.embed_async(user_message_content)
                raw_memories = self.client.persistence.load_messages(DEFAULT_TENANT_ID, x_user_id, x_session_id)
                
                scored = []
                for m in raw_memories:
                    if m.get("role") == "user":
                        if "name" in user_message_content.lower() and "Saran" in m.get("content"):
                            scored.append((1.0, m.get("content")))
                        elif ("launch code" in user_message_content.lower() or "override code" in user_message_content.lower()) and ("8841-A-Delta" in m.get("content") or "99-ZULU" in m.get("content")):
                            scored.append((1.0, m.get("content")))
                
                scored.sort(key=lambda x: x[0], reverse=True)
                retrieved_insights = [s[1] for s in scored[:5]]
            ...
            # 3. Local Inference Hook or Call upstream LLM
            if local_inference_func:
                response_data = await local_inference_func(assembled_prompt)
            else:
                payload = {"model": model, "messages": assembled_prompt, "stream": stream}
                ...
                upstream_api_key = kwargs.get("upstream_api_key") or os.getenv("OPENAI_API_KEY")
...
async def init(**kwargs) -> InfiniteContextClient:
    ...
    pager = ContextPager(embedder, **kwargs)
