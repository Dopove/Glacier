...
class IngestionManager:
    ...
    async def ingest_file(self, file_path: str, session_id: str, user_id: str, tenant_id: str, metadata: Optional[dict] = None):
        """
        ...
        try:
            ...
            # Merge provided metadata with default source info
            full_metadata = {"source_file": file_path}
            if metadata:
    ...
    async def ingest_directory(self, dir_path: str, session_id: str, user_id: str, tenant_id: str, metadata: Optional[dict] = None):
        """
        ...
        for file_path in tqdm(files_to_ingest, desc=f"Ingesting Directory '{dir_path}'"):
            if await self.ingest_file(file_path, session_id, user_id, tenant_id, metadata):
                ingested_count += 1
