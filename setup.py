...
def get_wheel_url():
    ...
    mamba_ssm_version = "2.3.2" # Base Mamba version for prebuilt wheels if checking
    ...
    wheel_filename = f"mamba_ssm-{mamba_ssm_version}+{cuda_or_hip}{gpu_compute_version}torch{torch_version}cxx11abi{cxx11_abi}-{python_version}-{python_version}-{platform_name}.whl"
...
class CachedWheelsCommand(_bdist_wheel):
    ...
    def run(self):
        # We always want to build from source for GLACIER to include all our custom code
        # Force build to ignore prebuilt mamba_ssm wheels, as they won't contain ice_lite
        print("GLACIER: Building from source to include all components...")
        super().run()
...
setup(
    ...
    py_modules=["temporal_rag", "advanced", "demo", "integration", "app", "test_glacier"],
    author="Saran S, Tri Dao, Albert Gu",
    author_email="saran@dopove.com, tri@tridao.me, agu@cs.cmu.edu",
